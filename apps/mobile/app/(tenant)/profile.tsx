  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.neutralLight, padding: spacing.xl }}>
      <BhausCard style={{ alignItems: 'center', gap: spacing.sm }}>
        <BhausAvatar fullName={user?.fullName ?? 'Tenant User'} imageUrl={user?.avatarUrl} size={64} />
        <BhausText style={{ fontWeight: '700' }}>{user?.fullName ?? 'Tenant User'}</BhausText>
        <BhausText variant="caption">{user?.email ?? 'tenant@email.com'}</BhausText>
